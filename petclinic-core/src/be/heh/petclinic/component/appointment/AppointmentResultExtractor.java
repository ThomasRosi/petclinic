...
public class AppointmentResultExtractor implements ResultSetExtractor<Appointment> {
    ...
    public Appointment extractData(ResultSet rs) throws SQLException {
        ...
        Vet vet = new Vet(rs.getString("v.last_name"),rs.getString("v.first_name"),null);//speciality is an entity in the database but just a string in the backend domain, maybe that can be more powerful to create a new class for it
        Appointment appointment = new Appointment(rs.getInt("a.id"),pet,vet,rs.getDate("a.visit_date"),rs.getInt("a.visit_duration"),rs.getString("a.description"));
        return appointment;
    }
}
